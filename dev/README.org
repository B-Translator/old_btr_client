...
Development Setup of Btranslator
...
* Installation
  ...
  mkdir /var/chroot
  cd /var/chroot/
  git clone https://github.com/dashohoxha/btr-client.git Btranslator
...
* Changing the webserver
  ...
  *Important:* When /apache2/ is started, the services /nginx/,
  ...
  #+BEGIN_EXAMPLE
  127.0.0.1  btranslator.org dev.btranslator.org
  #+END_EXAMPLE
...
* Re-installing the application
  ...
  the btranslator profile with *drush site-install*, and then do all the
...
* Making a clone for development
  ...
  application can be accessed at *dev.btranslator.org*.
  ...
  *Caution:* The root directory and the DB of the clone will be
  ...
  chroot /var/chroot/btr/
  cd /var/www/
  drush @btr php-script btr/profiles/btranslator/dev/config.php dev1
  drush @dev php-script btr/profiles/btranslator/dev/config.php dev2
  #+END_EXAMPLE

  It will set site_name to 'Btranslator (dev1)', will make site email
...
* Making a backup of the application
  ...
  chroot /var/chroot/btr/
  cd /var/www/
  ln -s btr_dev/profiles/btranslator/ Btranslator

  Btranslator/dev/snapshot.sh make @btr
  Btranslator/dev/snapshot.sh make @dev
  #+END_EXAMPLE
  These will create the files ~snapshot-btr-20130602.tgz~ and
  ~snapshot-btr_dev-20130602.tgz~. They can be restored like this:
  #+BEGIN_EXAMPLE
  Btranslator/dev/snapshot.sh restore @btr --file=snapshot-btr-20130602.tgz
  Btranslator/dev/snapshot.sh restore @btr --file=snapshot-btr_dev-20130602.tgz
  Btranslator/dev/snapshot.sh restore @dev --file=snapshot-btr-20130602.tgz
  Btranslator/dev/snapshot.sh restore @dev --file=snapshot-btr_dev-20130602.tgz
  #+END_EXAMPLE
...
* Pushing commits to github
  ...
  The copy of the application on =/var/www/btr_dev/profiles/btranslator/=
  (as well as the one on =/var/www/btr/profiles/btranslator/=) are actually
  clones of the git repository at https://github.com/dashohoxha/btr-client
...
* Making a local git clone of the dev application
  ...
  *@dev* application (~/var/www/btr_dev~). For example this is the
  ...
  [[https://github.com/dashohoxha/btr-client][GitHub repository]] it is easy to push commits from /btr_dev/ to
...
* Working with a dev-test-live workflow
  ...
  $aliases['live'] = array (
    'root' => '/var/www/btr',
    'uri' => 'http://btranslator.org',

    'remote-host' => 'btranslator.org',
    'remote-user' => 'root',
  ...
  $aliases['test'] = array (
    'parent' => '@live',
    'root' => '/var/www/btr',
    'uri' => 'http://test.btranslator.org',
    'remote-host' => 'test.btranslator.org',
