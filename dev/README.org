...
* Making a clone for development
  ...
  DB named *bcl_dev*. It also creates the drush alias *@bcl_dev*, and
  ...
  *Caution:* The root directory and the DB of the clone will be
  ...
  drush @bcl_dev php-script bcl/profiles/btr_client/config.php dev2
  #+END_EXAMPLE
...
* Making a backup of the application
  ...
  btr_client/dev/snapshot.sh make @bcl_dev
  #+END_EXAMPLE
  ...
  btr_client/dev/snapshot.sh restore @bcl_dev --file=snapshot-bcl-20130602.tgz
  btr_client/dev/snapshot.sh restore @bcl_dev --file=snapshot-bcl_dev-20130602.tgz
  #+END_EXAMPLE
...
* Making a local git clone of the dev application
  ...
  *@bcl_dev* application (~/var/www/bcl_dev~). For example this is the
