...
* Installation
  ...
  nohup nice B-Translator/install/install.sh bcl &
  tail -f nohup.out
...
* Making a backup of the application
  ...
  like this:
  #+BEGIN_EXAMPLE
  B-Translator/dev/snapshot.sh restore --file=snapshot-bcl-20130602.tgz
  B-Translator/dev/snapshot.sh restore --file=snapshot-bcl_dev-20130602.tgz
  B-Translator/dev/snapshot.sh restore @dev --file=snapshot-bcl-20130602.tgz
  B-Translator/dev/snapshot.sh restore @dev --file=snapshot-bcl_dev-20130602.tgz
  #+END_EXAMPLE
...
* Making a local git clone of the dev application
  ...
  *@dev* application (~/var/www/bcl_dev~). For example this is the
  ...
  [[https://github.com/dashohoxha/B-Translator][GitHub repository]], it is easy to push commits from /bcl_dev/ to
