#!/bin/bash
### Create a new Drupal project, using the project Btranslator as a
### template project.
###
### This new project, same as Btranslator, will contain a Drupal profile,
...
### template project they are represented by 'btranslator' and 'btr', which
...
### name and prefix ('btranslator' and 'btr'), and this helps to maintain
...
Create a new Drupal project, using the project Btranslator
as a template project.
    ...
    --source=S    source of the template project
                  (default https://github.com/B-Translator/client)
...
function get_options {
    ...
    source=${source:-https://github.com/B-Translator/client}
...
function get_template_project {
    ...
    rm -rf $project/.git
    rm -rf $project/modules/features/btranslator_objects/
    rm -rf $project/modules/custom/btranslatorlib/
}


### remove referencies to $branch, or change them to master
function rename_branch_to_master {
    sed -i $project/build-btranslator.make \
        -e "/\[branch\] = $branch/ s/^;*/;/"

    files="build-btranslator.make
           install/install-scripts/20-make-and-install-btranslator.sh"
    for file in $files
    do
         sed -i $project/$file \
             -e "s#/B-Translator/client/$branch/#/dashohoxha/dbox/master/#g"
    done

    files="README.org dev/README.org
           install/install-scripts/30-git-clone-btranslator.sh"
    for file in $files
    do
...
rename_all_files btranslator $project
rename_all_files btr $prefix

replace_in_all_files btranslator $project
replace_in_all_files Btranslator ${project^}
replace_in_all_files btr $prefix
